...
init_per_testcase(data, Config) ->
    ...
    ok = emqx_modules_registry:mnesia(boot),
    application:ensure_all_started(emqx_modules),
    ok = emqx_schema_registry:mnesia(boot),
    application:ensure_all_started(emqx_schema_registry),
    ok = emqx_conf:mnesia(boot),
    application:ensure_all_started(emqx_conf),
